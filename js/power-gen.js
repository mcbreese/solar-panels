    ...
    $("#submit").click(() => {
        ...
        if (getCustomerPostCode($("#customer").val())) {
            ...
            if (emptyFields()) {
                ...
                var htmlSummer =
                    "<h3>Output</h3><p style='text-decoration: underline;'>Summer Solstice</p> <br/> " +
                    " The installation cost of the panels = £" +
                    MTotal +
                    " <br/>" +
                    " Assumed maximum noon-time power generation = " +
                    PPanelSummer +
                    "W <br/>" +
                    " Cost of power generated at this time = £" +
                    powerCostSummer +
                    " per Watt";
                var htmlSummerBreakdown =
                    "<h3>Breakdown</h3><p style='text-decoration: underline';>Summer Solstice</p></br>Installation cost = " +
                    area +
                    " x " +
                    cost +
                    "<br/>" +
                    ...
                    PPanelSummer;
                var htmlWinter =
                    "<p style='text-decoration: underline;'>Winter Solstice</p><br/>" +
                    " The installation cost of the panels = £" +
                    MTotal +
                    " <br/>" +
                    " Assumed maximum noon-time power generation = " +
                    ...
                    powerCostWinter +
                    " per Watt";
                var htmlWinterBreakdown =
                    "<p style='text-decoration: underline;'>Winter Solstice</p><br/> Installation cost = " +
                    area +
                    " x " +
                    cost +
                    "<br/>" +
...
function angleOfSun(postcode, season) {
    ...
    fetch("http://api.getthedata.com/postcode/" + postcode)
        ...
        .then((data) => {
            ...
            fetch("https://api.meteomatics.com/" + dateTimeString + ":PT1H/sun_elevation:d/" + latLon[0] + "," + latLon[1] + "/json", { method: "get", headers: { "Content-Type": "application/json", Authorization: "basic " + credentials } })
                .then((response) => response.json())
                .then((data) => {
